...
yidata_df = pd.read_csv('yirobotweb3.csv')
print(yidata_df.head())

garfield_inaugural = yidata_df.iloc[5].Specification
...
print("closest_similarity_index")
print(closest_similarity_index)
print("010108")
...
def qa_speech_chat_completion(client, model, user_question, relevant_excerpts):
    chat_completion = client.chat.completions.create(
        messages = [
            {
                "role": "system",
                "content": "你是問答機器人(ignoring the chunk)." 
            },
            {
                "role": "user",
...
#qa_speech_chat_completion(client, model, user_question, most_relevant_chunk)
#Type,Type_spc,Specification,Product,Weburl
documents = []
for index, row in yidata_df[yidata_df['Product'].notnull()].iterrows():
    print("8888")
    print(row)
    print("888880000000")
...
qa_speech_chat_completion(client, model, user_question, relevant_excerpts)
